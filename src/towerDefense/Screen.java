...
import towerDefense.Clock;
import towerDefense.Frame;
...
public class Screen extends JPanel implements Runnable {
    ...
    public int status = 0;
    Frame frame;
    private towerdefense.User user;
    private towerdefense.Level level;
    private towerdefense.GameField gameField;
    ...
    public Screen(Frame frame) {
        this.frame = frame;
        towerdefense.MouseHandler mouseHandler = new towerdefense.MouseHandler(this);
        towerdefense.KeyHandler keyHandler = new towerdefense.KeyHandler(this);
    ...
    private void inGame(Graphics g) {
        if (status == 1) {
            ...
            for (int i = 0; i < 24; i++) {
                for (int j = 0; j < 14; j++) {
                    g.drawImage(towerdefense.gameEntity.gameTile.Grass.getImage(), i * 64, j * 64, null);
                ...
                g.drawImage(towerdefense.gameEntity.gameTile.NormalTower.getImage(), 168, 736, null);
                g.drawImage(towerdefense.gameEntity.gameTile.MachineGunTower.getImage(), 268, 736, null);
                g.drawImage(towerdefense.gameEntity.gameTile.SniperTower.getImage(), 368, 736, null);
            }
                ...
                switch (onHand) {
                    case 1:
                        if (isAble(mousePosX, mousePosY))
                            draw(g, mousePosX, mousePosY, towerdefense.gameEntity.gameTile.NormalTower.getRange(), towerdefense.gameEntity.gameTile.NormalTower.getPath(), exepting);
                        else
                            draw(g, mousePosX, mousePosY, towerdefense.gameEntity.gameTile.NormalTower.getRange(), towerdefense.gameEntity.gameTile.NormalTower.getPath(), warning);
                        break;
                    case 2:
                        if (isAble(mousePosX, mousePosY))
                            draw(g, mousePosX, mousePosY, towerdefense.gameEntity.gameTile.MachineGunTower.getRange(), towerdefense.gameEntity.gameTile.MachineGunTower.getPath(), exepting);
                        else
                            draw(g, mousePosX, mousePosY, towerdefense.gameEntity.gameTile.MachineGunTower.getRange(), towerdefense.gameEntity.gameTile.MachineGunTower.getPath(), warning);
                        break;
                    case 3:
                        if (isAble(mousePosX, mousePosY))
                            draw(g, mousePosX, mousePosY, towerdefense.gameEntity.gameTile.SniperTower.getRange(), towerdefense.gameEntity.gameTile.SniperTower.getPath(), exepting);
                        else
                            draw(g, mousePosX, mousePosY, towerdefense.gameEntity.gameTile.SniperTower.getRange(), towerdefense.gameEntity.gameTile.SniperTower.getPath(), warning);
                        break;
                }
            }
    ...
    public void loadGame() {
        isRunning = true;
        user = new towerdefense.User(this);
        user.creatPlayer();
        gameField = new towerdefense.GameField(level, user.player);
    }
    ...
    public void update() {
        if (status == 1) {
            if (Clock.getTotalTime() - gameField.getTimeLastWave() >= level.newWaveSpeed * Clock.deltaDelay()) {
                if (Clock.getTotalTime() - gameField.getTimeLastSpawn() >= level.spawnSpeed * Clock.deltaDelay())
    ...
    public void placeTower(int x, int y, int onHand) {
        if (isAble(x, y) && status == 1) {
            ...
            switch (onHand) {
                case 1:
                    user.player.coin -= towerdefense.gameEntity.gameTile.NormalTower.getCost();
                    System.out.println("YOU BOUGHT NORMAL TOWER, YOUR COIN: " + user.player.coin);
                    break;
                case 2:
                    user.player.coin -= towerdefense.gameEntity.gameTile.MachineGunTower.getCost();
                    System.out.println("YOU BOUGHT MACHINE GUN TOWER, YOUR COIN: " + user.player.coin);
                    break;
                case 3:
                    user.player.coin -= towerdefense.gameEntity.gameTile.SniperTower.getCost();
                ...
                if (mouseDown && onHand == 0) {
                    ...
                    if (e.getY() >= 728 & e.getY() <= 808) {
                        if (e.getX() >= 160 && e.getX() <= 240) {
                            if (user.player.coin >= towerdefense.gameEntity.gameTile.NormalTower.getCost()) {
                                onHand = 1;
                            } else
                                System.out.println("NOT ENOUGH MONEY!");
                        }
                        if (e.getX() >= 260 && e.getX() <= 340) {
                            if (user.player.coin >= towerdefense.gameEntity.gameTile.MachineGunTower.getCost()) {
                                onHand = 2;
                            } else
                                System.out.println("NOT ENOUGH MONEY!");
                        }
                        if (e.getX() >= 360 && e.getX() <= 440) {
                            if (user.player.coin >= towerdefense.gameEntity.gameTile.SniperTower.getCost()) {
                                onHand = 3;
                            } else
                                System.out.println("NOT ENOUGH MONEY!");
                        }
